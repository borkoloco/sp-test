...
const CsvUploader: React.FC = () => {
    ...
    try {
      if (file) {
        ...
        if (response.status === 200) {
          ...
          setFileInvalid(false); // Si el archivo se subió correctamente, no hay error
        }
      }
    } catch (error: any) {
      handleFileUploadError(error); // Si hay error, lo manejamos
    }
  };
  ...
  const handleFileUploadError = (error: any) => {
    if (error.response) {
      ...
      // Aquí capturamos el mensaje que devuelve el backend
      setMessageHere(
